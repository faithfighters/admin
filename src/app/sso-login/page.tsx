...
export default function SsoLoginPage() {
  ...
  useEffect(() => {
    // Read the query parameters using standard browser APIs inside useEffect
    // This avoids build-time Suspense warnings/errors from Next.js' useSearchParams
    ...
    if (!token) {
      ...
      // Automatically redirect to manual login after 2 seconds
    ...
    async function handleSso() {
      try {
        const response = await fetch(`/api/auth/sso?token=${encodeURIComponent(token!)}`);
        ...
        if (response.ok) {
          ...
          // Refresh user context in AuthProvider to update logged-in state locally
          await refreshUser();
