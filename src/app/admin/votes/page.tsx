    ...
    useEffect(() => {
        Promise.all([
            ...
            .then(([cycleData, causeData]) => {
                ...
                const causesList = causeData.causes || [];
                setAllCauses(causesList);
                // Pre-select all causes by default
                setSelectedCauseIds(causesList.map((c: Cause) => c.id));
                // Set default end date to November 30, 2030 (4 years later)
                setNewEnd('2030-11-30');
            })
            .catch(() => {})
    ...
    const handleCreate = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (res.ok) {
                ...
                flash('Failed to create cycle.', false);
            }
    ...
    const handleAddCause = async () => {
        ...
        try {
            ...
            if (res.ok) {
                ...
                flash('Failed to add cause.', false);
            }
    ...
    const handleRemoveCause = async (causeId: string) => {
        ...
        try {
            ...
            if (res.ok) {
                ...
                flash('Failed to remove cause.', false);
            }
        } finally { setSaving(false); }
    };

    if (loading) return <div style={{ padding: '3rem', textAlign: 'center', color: 'rgba(255,255,255,0.5)' }}>Loading cycles…</div>;
    ...
    return (
        <div>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', marginBottom: '28px' }}>
                <div>
                    <h1 style={{ fontSize: '28px', fontWeight: 800, color: '#ffffff', margin: 0 }}>Voting Cycles</h1>
                    <p style={{ color: 'rgba(255,255,255,0.5)', marginTop: '6px' }}>Manage donation voting cycles and cause allocations.</p>
                </div>
                {!activeCycle && (
                    <button
                        style={primaryBtn}
            ...
            {msg && (
                <div style={{
                    ...
                    background: msg.ok ? 'rgba(34,197,94,0.15)' : 'rgba(231,66,27,0.15)',
                    color: msg.ok ? '#4ade80' : '#F8C38F',
                    border: `1px solid ${msg.ok ? 'rgba(34,197,94,0.3)' : 'rgba(231,66,27,0.3)'}`,
                }}>
            ...
            {showNewCycle && (
                <form onSubmit={handleCreate} style={cardStyle}>
                    ...
                    <div style={{ marginBottom: '20px' }}>
                        ...
                        {allCauses.length === 0 ? (
                            <p style={{ fontSize: '13px', color: 'rgba(255,255,255,0.4)' }}>No active causes found. Create and approve causes first.</p>
                        ) : (
                                ...
                                {allCauses.map(c => {
                                    ...
                                    return (
                                        <label key={c.id} style={{
                                            ...
                                            border: `1.5px solid ${checked ? '#E7421B' : 'rgba(255,255,255,0.08)'}`,
                                            background: checked ? 'rgba(231,66,27,0.12)' : 'rgba(255,255,255,0.04)',
                                            fontSize: '13px', fontWeight: 600, color: checked ? '#F8C38F' : '#ffffff',
                                            ...
                                            <input
                                                ...
                                                style={{ accentColor: '#E7421B' }}
                                            />
                                            {c.name}
                                            <span style={{ fontSize: '12px', color: 'rgba(255,255,255,0.4)', fontWeight: 500 }}>({c.category})</span>
                                        </label>
                                    );
                                })}
                            </div>
            ...
            {activeCycle ? (
                <div style={cardStyle}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' }}>
                        ...
                        <span style={{ padding: '6px 14px', borderRadius: '12px', background: 'rgba(34,197,94,0.15)', color: '#4ade80', fontSize: '12px', fontWeight: 700 }}>
                            ACTIVE
                        </span>
                    </div>
                    ...
                    <div style={{ marginBottom: '20px' }}>
                        ...
                        {activeCausesIds.length === 0 ? (
                            <div style={{
                                display: 'flex', alignItems: 'center', gap: 8,
                                padding: '20px', borderRadius: '12px', background: 'rgba(245,158,11,0.12)',
                                border: '1px solid rgba(245,158,11,0.3)', color: '#fbbf24', fontSize: '13px', fontWeight: 600,
                            }}>
                                <AlertTriangle size={15} />No causes added yet. Add causes below so members can vote on them.
                            </div>
                        ) : (
                                ...
                                {(activeCycle.causes as any[]).map((c: any) => {
                                    ...
                                    return (
                                        <div key={id} style={{
                                            ...
                                            <div>
                                                <span style={{ fontSize: '14px', fontWeight: 700, color: '#ffffff' }}>{name}</span>
                                                {category && <span style={{ marginLeft: '8px', fontSize: '12px', color: 'rgba(255,255,255,0.5)', background: 'rgba(255,255,255,0.06)', padding: '2px 8px', borderRadius: '6px' }}>{category}</span>}
                                            </div>
                                            <button
                                                onClick={() => handleRemoveCause(id)}
                                                disabled={saving}
                                                style={{ border: 'none', background: 'rgba(231,66,27,0.15)', color: '#F8C38F', fontSize: '12px', fontWeight: 700, padding: '5px 12px', borderRadius: '8px', cursor: 'pointer' }}
                                            >
                                                Remove
                                            </button>
                                        </div>
                ...
                <div style={{
                    ...
                    <AlertTriangle size={16} />No active voting cycle. Click &quot;+ New Cycle&quot; above to create one — then members can cast their votes.
                </div>
            )}
            ...
            <div style={cardStyle}>
                ...
                {cycles.length === 0 ? (
                    <p style={{ color: 'rgba(255,255,255,0.4)', fontSize: '13px' }}>No cycles created yet.</p>
                ) : (
                    <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '13px' }}>
                        <thead>
                            <tr style={{ borderBottom: '1px solid rgba(255,255,255,0.06)' }}>
                                {['Cycle Name', 'Start', 'End', 'Causes', 'Status'].map(h => (
                                    <th key={h} style={{ padding: '10px 12px', textAlign: 'left', fontSize: '12px', fontWeight: 700, color: 'rgba(255,255,255,0.4)', textTransform: 'uppercase', letterSpacing: '0.5px' }}>{h}</th>
                                ))}
                            </tr>
                        </thead>
                        <tbody>
                            {cycles.map(cycle => (
                                <tr key={cycle.id} style={{ borderBottom: '1px solid rgba(255,255,255,0.04)' }}>
                                    <td style={{ padding: '12px', fontWeight: 700, color: '#ffffff' }}>{cycle.name}</td>
                                    <td style={{ padding: '12px', color: 'rgba(255,255,255,0.5)' }}>{new Date(cycle.startDate).toLocaleDateString()}</td>
                                    <td style={{ padding: '12px', color: 'rgba(255,255,255,0.5)' }}>{new Date(cycle.endDate).toLocaleDateString()}</td>
                                    <td style={{ padding: '12px', color: 'rgba(255,255,255,0.5)' }}>{(cycle.causes as any[]).length}</td>
                                    <td style={{ padding: '12px' }}>
                                        <span style={{
                                            padding: '4px 10px', borderRadius: '8px', fontSize: '12px', fontWeight: 700,
                                            background: cycle.status === 'active' ? 'rgba(34,197,94,0.15)' : 'rgba(255,255,255,0.06)',
                                            color: cycle.status === 'active' ? '#4ade80' : 'rgba(255,255,255,0.6)',
                                        }}>{cycle.status}</span>
                                    </td>
                                </tr>
                            ))}
...
const primaryBtn: React.CSSProperties = {
    padding: '10px 22px', background: '#E7421B', color: '#fff', border: 'none',
    borderRadius: '12px', fontSize: '13px', fontWeight: 700, cursor: 'pointer',
    transition: 'all 0.2s', whiteSpace: 'nowrap',
};

const cardStyle: React.CSSProperties = {
    background: '#15131f', border: '1px solid rgba(255,255,255,0.06)', borderRadius: '20px',
    padding: '28px', marginBottom: '24px',
    boxShadow: '0 2px 8px rgba(0,0,0,0.3)',
};

const cardTitle: React.CSSProperties = {
    fontSize: '18px', fontWeight: 800, color: '#ffffff', margin: '0 0 16px 0',
};

const labelStyle: React.CSSProperties = {
    display: 'block', fontSize: '12px', fontWeight: 700, color: 'rgba(255,255,255,0.85)', marginBottom: '6px',
};

const inputStyle: React.CSSProperties = {
    width: '100%', padding: '10px 14px', border: '1.5px solid rgba(255,255,255,0.08)', borderRadius: '10px',
